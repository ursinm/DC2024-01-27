...
public class NoteService : INoteService
{
    ...
    public async Task<NoteResponseDto> GetNoteByIdAsync(NoteKey id)
    {
    ...
    public async Task<NoteResponseDto> UpdateNoteAsync(NoteRequestDto note)
    {
        ...
        var n = _autoMapper.Map<Note>(note);
        NoteKey key = _autoMapper.Map<NoteKey>(n);
        var res = await _noteRepository.UpdateAsync(key,_autoMapper.Map<Note>(note));
        return _autoMapper.Map<NoteResponseDto>(res);
    }

    public async Task DeleteNoteAsync(NoteKey id)
    {
