...
public class NewsServiceImpl implements NewsService {
    ...
    public NewsResponseDto create(@Valid @NonNull NewsRequestDto dto) throws EntityExistsException {
        Optional<News> creator = dto.getId() == null ? Optional.empty() : newsRepository.findById(dto.getId());
        if (creator.isEmpty()) {
    ...
    public Optional<NewsResponseDto> read(@NonNull Long id) {
        return newsRepository.findById(id).flatMap(creator -> Optional.of(
                newsResponseConverter.toDto(creator)));
    }

    @Override
    @Validated
    public NewsResponseDto update(@Valid @NonNull NewsRequestDto dto) throws NoEntityExistsException {
        Optional<News> creator = dto.getId() == null || newsRepository.findById(dto.getId()).isEmpty() ?
                Optional.empty() : Optional.of(newsRequestConverter.fromDto(dto));
        return newsResponseConverter.toDto(newsRepository.save(creator.orElseThrow(() ->
    ...
    public Long delete(@NonNull Long id) throws NoEntityExistsException {
        Optional<News> creator = newsRepository.findById(id);
        newsRepository.deleteById(creator.map(News::getId).orElseThrow(() ->
                new NoEntityExistsException(Messages.NoEntityExistsException)));
