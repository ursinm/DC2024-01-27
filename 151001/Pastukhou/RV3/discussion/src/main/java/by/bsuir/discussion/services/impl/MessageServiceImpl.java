...
public class MessageServiceImpl implements MessageService {
    ...
    public Optional<MessageResponseDto> read(@NonNull Long id) {
        return messageRepository.findMessageById(id).flatMap(creator -> Optional.of(
                messageResponseConverter.toDto(creator)));
    }

    @Override
    @Validated
    public MessageResponseDto update(@Valid @NonNull MessageRequestDto dto) throws NoEntityExistsException {
        Optional<Message> message = dto.getId() == null || messageRepository.findMessageByTweetIdAndId(
