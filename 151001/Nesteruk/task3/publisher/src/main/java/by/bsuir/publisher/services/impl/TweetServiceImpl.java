...
public class TweetServiceImpl implements TweetService {
    ...
    public TweetResponseDto create(@Valid @NonNull TweetRequestDto dto) throws EntityExistsException {
        Optional<Tweet> issue = dto.getId() == null ? Optional.empty() : tweetRepository.findById(dto.getId());
        if (issue.isEmpty()) {
    ...
    public Optional<TweetResponseDto> read(@NonNull Long id) {
        return tweetRepository.findById(id).flatMap(issue -> Optional.of(
                tweetResponseConverter.toDto(issue)));
    }

    @Override
    @Validated
    public TweetResponseDto update(@Valid @NonNull TweetRequestDto dto) throws NoEntityExistsException {
        Optional<Tweet> issue = dto.getId() == null || tweetRepository.findById(dto.getId()).isEmpty() ?
                Optional.empty() : Optional.of(tweetRequestConverter.fromDto(dto));
        return tweetResponseConverter.toDto(tweetRepository.save(issue.orElseThrow(() ->
    ...
    public Long delete(@NonNull Long id) throws NoEntityExistsException {
        Optional<Tweet> issue = tweetRepository.findById(id);
        tweetRepository.deleteById(issue.map(Tweet::getId).orElseThrow(() ->
                new NoEntityExistsException(Messages.NoEntityExistsException)));
