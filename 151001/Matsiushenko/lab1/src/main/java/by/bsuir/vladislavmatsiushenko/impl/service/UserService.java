...
import by.bsuir.vladislavmatsiushenko.api.UserMapper;
import by.bsuir.vladislavmatsiushenko.impl.repository.UserRepository;
...
public class UserService implements Service<UserResponseTo, UserRequestTo> {
    ...
    private boolean validateUser(User user) {
        ...
        return  (firstname.length() >= 2 && firstname.length() <= 64) &&
                (lastname.length() >= 2 && lastname.length() <= 64) &&
                (password.length() >= 8 && firstname.length() <= 128) &&
                (login.length() >= 2 && login.length() <= 64);
    }
}
