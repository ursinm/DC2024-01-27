...
namespace Test
{
    ...
    {
        private readonly Mock<ILogger<CreatorController>> _loggerMock = new();
        ...
        private readonly CreatorService _authorService;
        private readonly CreatorController _authorController;

        public AuthorControllerUnitTest()
        {
            _authorService = new CreatorService(_context, _mapper);
            _authorController = new CreatorController(_authorService, _loggerMock.Object, _mapper)
            {
        ...
        public void GetAll()
        {
            ...
            Assert.IsInstanceOfType(result.Value, typeof(List<CreatorResponseTO>));
        }

        [TestMethod]
        public async Task GetByTweetID()
        {
            var authorRequest = new CreatorRequestTO(0, "author", "passwrod", "fname", "lname");
            var authorResponse = (await _authorController.Create(authorRequest)).Value as CreatorResponseTO;
            Assert.IsNotNull(authorResponse);

            var tweetRequest = new NewsRequestTO(0, authorResponse.Id, "title", "content", DateTime.Now, DateTime.Now);

            var tweetService = new NewsService(_context, _mapper);
            var tweetController = new NewsController(tweetService, new Mock<ILogger<NewsController>>().Object, _mapper)
            {
                ControllerContext = new ControllerContext { HttpContext = new DefaultHttpContext() }
            };

            var tweetResponse = (await tweetController.Create(tweetRequest)).Value as NewsResponseTO;
            Assert.IsNotNull(tweetResponse);

            var result = await _authorController.GetByNewsID(tweetResponse.Id);
        ...
        {
            var authorRequest = new CreatorRequestTO(0, "testDeleteLogin", "password", "fname", "lname");
            var authorResponse = (await _authorController.Create(authorRequest)).Value as CreatorResponseTO;
            Assert.IsNotNull(authorResponse);
        ...
        public async Task Patch()
        {
            ...
            var patch = new JsonPatchDocument<Creator>();
            var addOperation = new Operation<Creator>
            {
                op = "add",
                path = "/FirstName",
                value = expected
            ...
            var result = (await _authorController.PartialUpdate(authorResponse.Id, patch)).Value as CreatorResponseTO;
