...
public class PostService {
    ...
    public void deletePost(Long id) throws DeleteException {
        ...
        if (post.isEmpty()) {
            ...
            postDao.deleteByCountryAndTweetIdAndId(post.get().getCountry(), post.get().getTweetId(), post.get().getId());
        }
    }
    ...
    public PostResponseTo updatePost(@Valid PostRequestTo post, String country) throws UpdateException {
        ...
        if (postOptional.isEmpty()) {
            ...
            postDao.deleteByCountryAndTweetIdAndId(postOptional.get().getCountry(), postOptional.get().getTweetId(), postOptional.get().getId());
            return postToPostResponse(postDao.save(postToUpdate));
        }
    }

    public List<PostResponseTo> getPostByTweetId(Long tweetId) throws NotFoundException {
        List<Post> post = postDao.findByTweetId(tweetId);
        if (post.isEmpty()) {
    ...
    private PostResponseTo postToPostResponse(Post post) {
        ...
        postResponseTo.setTweetId(post.getTweetId());
    ...
    private Post postRequestToPost(PostRequestTo requestTo) {
        ...
        post.setTweetId(requestTo.getTweetId());
        return post;
    }
