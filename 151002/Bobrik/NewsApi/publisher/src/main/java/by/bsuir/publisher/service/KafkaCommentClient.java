...
import by.bsuir.publisher.service.exceptions.CommentExchangeTimeoutException;
...
public class KafkaCommentClient {
    ...
    private static long hashMessage(InTopicMessage message) {
        if (message == null)
            return 0;
        final CommentInTopicTo dto = message.commentDto();
        if (dto == null)
            return 0;
    ...
    public OutTopicMessage sync(InTopicMessage message) {
        ...
        try {
            ...
            if (result instanceof OutTopicEvent outEvent) {
                ...
                throw new CommentExchangeFailedException("Invalid comment response data");
            }
        } catch (TimeoutException ex) {
            kafkaExchangers.remove(id);
            throw new CommentExchangeTimeoutException("Response time has expired", ex);
        } catch (InterruptedException ex) {
            throw new CommentExchangeFailedException("Comment data request failed", ex);
        }
    }
