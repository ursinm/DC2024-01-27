package by.bsuir.publisher.listener;

import by.bsuir.publisher.event.*;
import by.bsuir.publisher.exception.PostExchangeFailedException;
import by.bsuir.publisher.exception.PostExchangeTimeoutException;
...
public class KafkaPostListener {
    ...
    private static long hashMessage(InTopicMessage message) {
        if (message == null)
            return 0;
        final PostInTopicDto dto = message.postDto();
        if (dto == null)
            return 0;
    ...
    public OutTopicMessage sync(InTopicMessage message) {
        ...
        try {
            ...
            if (result instanceof OutTopicEvent outEvent) {
                ...
                throw new PostExchangeFailedException("Invalid post response data");
            }
        } catch (TimeoutException ex) {
            kafkaExchangers.remove(id);
            throw new PostExchangeTimeoutException("Response time has expired", ex);
        } catch (InterruptedException ex) {
            throw new PostExchangeFailedException("Post data request failed", ex);
        }
    }
