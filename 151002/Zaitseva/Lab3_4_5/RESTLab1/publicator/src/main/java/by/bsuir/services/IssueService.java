...
public class IssueService {
    ...
    public IssueResponseTo saveIssue(@Valid IssueRequestTo issue) throws DuplicationException {
        ...
        if (issue.getCreatorId() != null) {
            issueToSave.setCreator(creatorRepository.findById(issue.getCreatorId()).orElseThrow(() -> new NotFoundException("Creator not found!", 40004L)));
        }
    ...
    public List<IssueResponseTo> getIssueByCriteria(List<String> stickerName, List<Long> stickerId, String creatorLogin, String title, String content) {
        return issueListMapper.toIssueResponseList(issueDao.findIssuesByParams(stickerName, stickerId, creatorLogin, title, content));
    }
}
