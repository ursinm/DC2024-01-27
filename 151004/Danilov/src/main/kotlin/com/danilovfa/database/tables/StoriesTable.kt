...
object StoriesTable : Table(), StoriesDao {
    override val tableName: String = "tbl_stories"
    ...
    override suspend fun getStory(id: Long): Story? = dbExec {
        select { this@StoriesTable.id eq id }
            .map { it.toStory() }
            .singleOrNull()
    }
    ...
    override suspend fun insertStory(
        ...
        insert {
            it[this@StoriesTable.userId] = userId
            it[this@StoriesTable.title] = title
            it[this@StoriesTable.content] = content
            it[this@StoriesTable.created] = currentTime()
            it[this@StoriesTable.modified] = currentTime()
        } get id
    }
    ...
    override suspend fun updateStory(
        ...
        dbExec {
            update(
                where = { this@StoriesTable.id eq id }
            ) {
                it[this@StoriesTable.userId] = userId
                it[this@StoriesTable.title] = title
                it[this@StoriesTable.content] = content
    ...
    override suspend fun deleteStory(id: Long): Boolean = dbExec {
        deleteWhere { this@StoriesTable.id eq id } > 0
    }

    override suspend fun storyExists(title: String): Boolean = dbExec {
        select { this@StoriesTable.title eq title }
            .map { it.toStory() }
            .singleOrNull() != null
    }
