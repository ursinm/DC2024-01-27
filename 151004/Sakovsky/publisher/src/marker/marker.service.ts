...
export class MarkerService {
    constructor(@InjectRepository(Marker) 
        private markerRepositoty: Repository<Marker>){}
    ...
    async updateMarker(markerDto: MarkerRequestToUpdate): Promise<Marker>{
        const marker = await this.markerRepositoty.findOneBy({id: markerDto.id})
        if (!marker){
            throw new NotFoundException(`Marker with id: ${markerDto.id} not found.`)
        }
