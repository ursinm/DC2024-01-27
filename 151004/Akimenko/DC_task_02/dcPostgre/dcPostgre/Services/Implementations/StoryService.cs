...
namespace WebApplicationDC1.Services.Implementations
{
    public class StoryService(ApplicationContext applicationContext, IMapper mapper) : IStoryService
    {
        ...
        public async Task<StoryResponseTO> Add(StoryRequestTO story)
        {
            ...
            // Проверяем, существует ли уже история с таким же названием для этого автора
            var existingStory = await _context.Stories
                .FirstOrDefaultAsync(s => s.Title == story.Title);

            if (existingStory != null)
            {
                throw new InvalidOperationException("Story with this title already exists");
            }
        ...
        public async Task<StoryResponseTO> Update(StoryRequestTO Story)
        {
            ...
            existingStory.Modified = DateTime.Now.ToUniversalTime();
