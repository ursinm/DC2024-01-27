...
namespace REST.Service.Implementation
{
    public class CreatorService(DbStorage dbStorage, IMapper mapper) : ICreatorService
    {
        ...
        {
            var c = _mapper.Map<Creator>(creator);
            ...
            return _mapper.Map<CreatorResponseTO>(c);
        }
        ...
        public async Task<CreatorResponseTO> GetByID([FromRoute] int id)
        {
            ...
            return c is not null ? _mapper.Map<CreatorResponseTO>(c)
                : throw new ArgumentNullException($"Not found CREATOR {id}");
        }

        public async Task<CreatorResponseTO> GetByIssueID(int issueId)
        {
            var response = await _context.Issues.Include(t => t.Creator).FirstAsync(t => t.Id == issueId);
