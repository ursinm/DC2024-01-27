...
{
    public class PostsService(IMapper _mapper, BaseContext dbContext, IConsumer<Ignore, string> consumer, IProducer<Null, string> producer) : IPost
    {
        ...
        public async Task<IResponseTo> CreateEnt(IRequestTo Dto)
        {
            ...
            if (Dto is PostRequestTo postRequest)
            {
                ...
                post = new Message<Null, string> { Value = json };
                producer.ProduceAsync("InTopic", post).GetAwaiter().GetResult();
