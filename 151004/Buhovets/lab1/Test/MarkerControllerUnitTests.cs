...
namespace Test
{
    ...
    public class MarkerControllerUnitTests
    {
        ...
        private readonly StickerService _markerService;
        private readonly MarkerController _markerController;

        public MarkerControllerUnitTests()
        {
            _markerService = new StickerService(_context, _mapper);
        ...
        public void GetAll()
        {
            ...
            Assert.IsInstanceOfType(result.Value, typeof(List<StickerResponseTO>));
        }

        [TestMethod]
        public async Task Delete()
        {
            var markerRequest = new StickerRequestTO(0, "testMarkerDeleteName");
            var markerResponse = (await _markerController.Create(markerRequest)).Value as StickerResponseTO;
        ...
        public async Task Patch()
        {
            ...
            var patch = new JsonPatchDocument<Sticker>();
            var addOperation = new Operation<Sticker>
            {
                op = "add",
                path = "/Name",
                value = expected
            ...
            var result = (await _markerController.PartialUpdate(markerResponse.Id, patch)).Value as StickerResponseTO;
            Assert.IsNotNull(result);
