...
{
    public class MessagesService(IMapper _mapper, BaseContext dbContext, IConsumer<Ignore, string> consumer, IProducer<Null, string> producer, IDatabase redisDB) : IMessageService
    {
        ...
        public async Task<IResponseTo> CreateEnt(IRequestTo Dto)
        {
            ...
            if (Dto is MessageRequestTo messageRequest)
            {
                ...
                //
                redisDB.StringSet(messageRequest.Id.ToString(), json);
                //

                message = new Message<Null, string> { Value = messageRequest.Id.ToString() };
                producer.ProduceAsync("InTopic", message).GetAwaiter().GetResult();
                ...
                //
                string value = redisDB.StringGet(messageRequest.Id.ToString());                                
                //

                var messageResponseTo = JsonConvert.DeserializeObject<MessageResponseTo>(value);
